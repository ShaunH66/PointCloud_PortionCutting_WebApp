...
# Version: 1.2 (Features: File Uploads (CSV/XYZ/PCD/PYL/XLSX/XLS), Test Data Gen,
...
DEFAULT_BLADE_THICKNESS = 0.0 # mm
...
def load_point_cloud(uploaded_file):
    """Loads point cloud data from CSV, XYZ, PCD, PLY, XLSX, or XLS."""
    ...
    try:
        ...
        if file_ext == 'csv':
            ...
            try:
                ...
                else: # Fallback: Assume first 3 columns
                    if df_excel.shape[1] >= 3:
                         st.warning("Columns 'x', 'y', 'z' not found. Assuming first 3 columns.")
                         df = df_excel.iloc[:, 0:3].copy(); df.columns = ['x', 'y', 'z']
                    else: raise ValueError("Not enough columns in Excel file.")
            except ImportError: st.error("Reading Excel requires `openpyxl`. Install: `pip install openpyxl`"); return None
            except Exception as excel_error: st.error(f"Error reading Excel: {excel_error}"); return None
        else:
        ...
        st.toast(f"Loaded {len(df)} points from {uploaded_file.name}", icon="✅") # Use toast
        return df
        ...
        if num_slice_points >= 2:
            ...
            if num_slice_points >= MIN_POINTS_FOR_HULL and np.ptp(slice_points[['x','z']].values, axis=0).min() > FLOAT_EPSILON * 10:
                try:
                    ...
                    if assume_flat_bottom: points_for_hull = np.vstack((points_for_hull, [[slice_min_x, 0.0], [slice_max_x, 0.0]]))
                ...
                except (QhullError, ValueError, Exception): pass # Fallback on error
...
def calculate_cut_portions_reversed(
    ...
    results = { "portions": [], "total_volume": 0.0, "density": 0.0, "status": "Init Error", "calc_time": 0.0, "volume_profile": {}, "sorted_y_starts": np.array([]), "calc_start_y": 0.0, "calc_end_y": 0.0 }
    ...
    if target_portion_weight_g <= FLOAT_EPSILON: results["status"] = "Error: Target weight must be positive."; return results
    if slice_increment_mm <= FLOAT_EPSILON: results["status"] = "Error: Slice Thickness must be positive."; return results
    ...
    if loaf_length <= FLOAT_EPSILON: results["status"] = "Error: Loaf length near zero."; return results
    calc_start_y = loaf_min_y + start_trim_mm; calc_end_y = loaf_max_y - end_trim_mm
    results["calc_start_y"] = calc_start_y; results["calc_end_y"] = calc_end_y
    if calc_start_y >= calc_end_y - FLOAT_EPSILON: results["status"] = f"Error: Trims overlap/exceed loaf length."; return results

    try: # Wrap main calculation steps
        # --- 1. Calculate FULL Volume Profile ---
        volume_profile_start = time.time()
        full_y_steps = np.arange(loaf_min_y, loaf_max_y, slice_increment_mm)
        progress_bar = st.progress(0.0); num_steps = len(full_y_steps) if len(full_y_steps) > 0 else 1
        ...
        for i, y_start in enumerate(full_y_steps):
            ...
            y_end = min(y_start + slice_increment_mm, loaf_max_y)
        ...
        if total_volume_mm3 <= FLOAT_EPSILON * 100: raise ValueError(f"TOTAL volume ({total_volume_mm3:.2e} mm³) near zero.")
        results["total_volume"] = total_volume_mm3

        # --- 2. Calculate Density ---
        density_g_mm3 = total_weight_g / total_volume_mm3
        if not np.isfinite(density_g_mm3) or density_g_mm3 <= 0: raise ValueError("Density invalid.")
        results["density"] = density_g_mm3

        # --- 3. Iterate Backwards within TRIMMED Range ---
        cutting_start_time = time.time()
        current_portion_weight_accum = 0.0; last_cut_y = calc_end_y
        sorted_y_starts_fwd = np.array(sorted(volume_profile.keys()))
        results["sorted_y_starts"] = sorted_y_starts_fwd; results["volume_profile"] = volume_profile # Store for plotting
        relevant_y_indices = np.where((sorted_y_starts_fwd >= calc_start_y - slice_increment_mm) & (sorted_y_starts_fwd < calc_end_y))[0]
        if len(relevant_y_indices) == 0: raise ValueError("No slices within trim range.")
        ...
        for y_start in sorted_y_starts_rev_trimmed:
            if y_start < calc_start_y - FLOAT_EPSILON: continue
            ...
            if current_portion_weight_accum + slice_weight >= target_weight_lower_bound - FLOAT_EPSILON:
                ...
                if not no_interpolation:
                    ...
                    cut_y = max(cut_y, calc_start_y)
                portion_start_y = last_cut_y; portion_end_y = cut_y
                portion_length = max(0.0, portion_start_y - portion_end_y)
                actual_weight = 0.0
                if portion_length > FLOAT_EPSILON:
                    ...
                    if np.isnan(actual_vol): raise ValueError("Invalid vol during weight recalc.")
                ...
                last_cut_y = max(calc_start_y, portion_end_y - blade_thickness_mm) # Apply kerf, respect trim
        ...
        if last_cut_y > calc_start_y + FLOAT_EPSILON:
            first_piece_start_y, first_piece_end_y = last_cut_y, calc_start_y
            ...
            if np.isnan(first_piece_volume): raise ValueError("Invalid volume for waste.")
        ...
        if portions_temp:
            ...
            # Format Waste
            csy,cey,ln,wt = waste_piece_raw.values(); sy,ey = (min(csy,cey),max(csy,cey)) if np.isfinite(csy) and np.isfinite(cey) else (0,0)
            ...
            for i, p in enumerate(target_pieces_sorted):
                csy,cey,ln,wt = p.values(); sy,ey = (min(csy,cey),max(csy,cey)) if np.isfinite(csy) and np.isfinite(cey) else (0,0)
        ...
        status_message = f"Calculation complete. Found {len(output_portions)} portions within trim range (Portion 1 is waste)." if output_portions else "Warning: No portions calculated within trim range."

    except OverflowError: status_message = "Error: Numerical overflow."
    except ValueError as e: status_message = f"Error: {e}"
    except Exception as e: status_message = f"Unexpected error: {e}"; raise
    finally:
        ...
        results["portions"] = output_portions; results["status"] = status_message; results["calc_time"] = total_calc_time
        # Volume profile, density, sorted_y, calc ranges already stored
...
def plot_3d_loaf(points_df, portions=None, title="Cheese Loaf Point Cloud"):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter3d(x=points_df['x'], y=points_df['y'], z=points_df['z'], mode='markers', marker=dict(size=1.5, color=points_df['z'], colorscale='YlOrBr', opacity=0.7, colorbar=dict(title='Height (Z)')), name='Point Cloud' ))
    if portions and len(portions) > 0:
        ...
        plane_x = [min_x - 0.05 * x_rng, max_x + 0.05 * x_rng]; plane_z = [min_z - 0.05 * z_rng, max_z + 0.05 * z_rng]
        cut_locations = [p['cut_y'] for p in portions[:-1]]
        for i, cut_y in enumerate(cut_locations):
             if np.isfinite(cut_y):
                 fig.add_trace(go.Mesh3d(x=[plane_x[0], plane_x[1], plane_x[1], plane_x[0]], y=[cut_y, cut_y, cut_y, cut_y], z=[plane_z[0], plane_z[0], plane_z[1], plane_z[1]], opacity=0.4, color='red', alphahull=0, name=f"Cut after P{i+1}", showlegend= i == 0 ))
...
def plot_area_profile(volume_profile, sorted_y_starts, slice_increment_mm, calc_start_y=None, calc_end_y=None):
    ...
    plot_min_y = y_values[0] if len(y_values) > 0 else 0; plot_max_y = y_values[-1] + slice_increment_mm if len(y_values) > 0 else 0
    if calc_start_y is not None and calc_start_y > plot_min_y: fig.add_vrect(x0=plot_min_y, x1=calc_start_y, fillcolor="grey", opacity=0.15, layer="below", line_width=0, name="Start Trim")
    if calc_end_y is not None and calc_end_y < plot_max_y: fig.add_vrect(x0=calc_end_y, x1=plot_max_y, fillcolor="grey", opacity=0.15, layer="below", line_width=0, name="End Trim")
...
def plot_cumulative_weight(volume_profile, sorted_y_starts, density_g_mm3, portions, target_weight, tolerance, calc_start_y=None, calc_end_y=None):
    ...
    if sorted_y_starts.size > 0:
        y_coords.append(sorted_y_starts[0]); cumulative_weights.append(0.0)
        for i, y_start in enumerate(sorted_y_starts):
            slice_vol = volume_profile.get(y_start, 0.0); current_weight += (slice_vol * density_g_mm3)
    ...
    if tolerance > 0: fig.add_hline(y=target_weight + tolerance, line_dash="dot", line_color="orange", name="+ Tolerance"); fig.add_hline(y=target_weight - tolerance, line_dash="dot", line_color="orange", name="- Tolerance")
    ...
    # Add shaded regions for trims
    plot_min_y = y_coords[0] if y_coords else 0; plot_max_y = y_coords[-1] if y_coords else 0
    if calc_start_y is not None and calc_start_y > plot_min_y: fig.add_vrect(x0=plot_min_y, x1=calc_start_y, fillcolor="grey", opacity=0.15, layer="below", line_width=0, name="Start Trim")
    if calc_end_y is not None and calc_end_y < plot_max_y: fig.add_vrect(x0=calc_end_y, x1=plot_max_y, fillcolor="grey", opacity=0.15, layer="below", line_width=0, name="End Trim")
...
# -----------------------------------------------------------------------------
st.set_page_config(layout="wide", page_title="Cheese Portion Calc v1.2", page_icon="🧀")

# --- Initialize Session State ---
default_state = {'point_cloud_data': None, 'data_origin': None, 'last_file_id': None, 'calc_results': None }
...
with st.sidebar:
    ...
    data_source = st.radio("Select Source:", ("Generate Test Data", "Upload File"), index=0, key="data_source", label_visibility="collapsed")
    uploaded_file = None
    file_types = ['csv', 'xyz']
    ...
    if st.session_state.data_source == "Upload File":
        ...
        if ('pcd' in file_types or 'ply' in file_types) and not _open3d_installed: st.warning("Install `open3d` for .pcd/.ply support.")
        st.caption("Excel: Use columns 'x', 'y', 'z' (case-insensitive) on first sheet.")
    else: st.markdown("_(Using generated test data)_")

    # --- Core Weights ---
    st.subheader("2. Weight Settings")
    ...
    st.number_input("Target Weight Tolerance (+/- g)", min_value=0.0, value=DEFAULT_WEIGHT_TOLERANCE, step=0.5, format="%.1f", key="weight_tolerance", help="Allowed deviation from target weight (+/- grams).")
    ...
    st.number_input("Start Trim (mm)", min_value=0.0, value=DEFAULT_START_TRIM, step=1.0, format="%.1f", key="start_trim", help="Length to discard from start (min Y) before portioning.")
    st.number_input("End Trim (mm)", min_value=0.0, value=DEFAULT_END_TRIM, step=1.0, format="%.1f", key="end_trim", help="Length to discard from end (max Y) before portioning.")
    ...
    st.toggle("Top-Down Scan Only", value=False, key="top_down_scan", help="ON: Assumes only top Z known. Overrides 'Flat Bottom'. 1 Scan Setup only.")
    flat_bottom_disabled = st.session_state.get('top_down_scan', False)
    st.toggle("Top & Side Scan Only", value=st.session_state.get('flat_bottom', False), key="flat_bottom", help="ON: Assumes Z=0 bottom. Ignored if 'Top-Down Scan' is ON. 2 Scan Setup only.", disabled=flat_bottom_disabled)
    st.number_input("Downsample Voxel Size (mm)", min_value=0.0, value=DEFAULT_VOXEL_SIZE, step=0.5, format="%.1f", key="voxel_size", help="0 = OFF. Reduces points for speed. Requires 'open3d'.")
    if st.session_state.voxel_size > 0 and not _open3d_installed: st.warning("Downsampling requires `open3d`.")
...
# --- Main Area ---
st.title("🔪 Cheese Loaf Portion Calculator (v1.2)")
...
if calc_button or refresh_data:
    st.session_state.calc_results = None # Clear previous results
    ...
    if st.session_state.data_source == "Generate Test Data":
         with st.spinner("Generating test point cloud..."):
            ...
            if temp_points_df is not None: st.toast(f"Generated {len(temp_points_df)} points.", icon="✨")
            else: st.error("Failed to generate test data.")
    elif st.session_state.data_source == "Upload File" and uploaded_file is not None:
        with st.spinner(f"Loading point cloud from {uploaded_file.name}..."):
            temp_points_df = load_point_cloud(uploaded_file) # Toast is inside load_point_cloud
    ...
    if temp_points_df is not None and not temp_points_df.empty and st.session_state.voxel_size > 0:
        if _open3d_installed:
            with st.spinner(f"Downsampling (Voxel Size: {st.session_state.voxel_size:.1f} mm)..."):
                ...
                if downsampled_points.shape[0] > 0:
                    ...
                    st.toast(f"Downsampled to {len(st.session_state.point_cloud_data)} points.", icon="📦")
                else:
...
# This section remains outside the calculation trigger so the plot always shows if data exists
...
if points_df is not None and not points_df.empty:
    ...
    try: # Display basic dimensions
        ...
        else: col1.metric("Points Loaded", f"{len(points_df):,}"); col2.warning("Non-finite data.")
    except Exception as e: st.warning(f"Could not get dimensions: {e}")

    # Plot the 3D view - show cuts if they exist in state
    with st.spinner("Generating 3D plot..."):
        plot_title_3d = "Point Cloud with Calculated Cuts" if st.session_state.calc_results and st.session_state.calc_results.get("portions") else "Loaded Point Cloud"
        plot_portions_3d = st.session_state.calc_results.get("portions") if st.session_state.calc_results else None
        fig3d = plot_3d_loaf(points_df, portions=plot_portions_3d, title=plot_title_3d)
...
if calc_results:
    ...
    if portions:
        # --- Styling Function ---
        def highlight_waste_and_tolerance(df_display):
            target = st.session_state.target_weight
            tolerance = st.session_state.weight_tolerance
            styles = pd.DataFrame('', index=df_display.index, columns=df_display.columns)
            styles.iloc[0] = 'background-color: #FFF3CD; font-weight: bold;' # Style Portion 1 (waste)
            if tolerance > 0 and len(df_display)>1:
                 try: # Protect against non-numeric data in Est. Weight column
                     weight_col_idx = df_display.columns.get_loc("Est. Weight (g)")
                     weights_numeric = pd.to_numeric(df_display.iloc[1:, weight_col_idx], errors='coerce')
                     outside_tol_mask = (weights_numeric < target - tolerance) | (weights_numeric > target + tolerance)
                     styles.iloc[1:][outside_tol_mask.fillna(False).values] = 'background-color: #F8D7DA;' # Style outside tolerance
                 except KeyError: pass # Ignore if column name mismatch
                 except Exception: pass # Ignore other potential errors during styling
            return styles

        # --- Use Tabs for Output ---
        tab1, tab2 = st.tabs(["📊 Results Summary", "📈 Analysis Plots"])

        with tab1: # Results Table and Metrics
            ...
            with col_table:
                 st.dataframe(results_df.style.apply(highlight_waste_and_tolerance, axis=None), use_container_width=True, hide_index=True)
            ...
            with col_metrics:
                 ...
                 st.metric("Tolerance", f"± {st.session_state.weight_tolerance:.{DISPLAY_PRECISION}f} g", help="Allowed weight deviation.")
                 st.metric("Portion 1 Weight", f"{portions[0].get('weight', 0.0):.{DISPLAY_PRECISION}f} g", help="Waste piece weight.")
                 ...
                 st.metric("Total Calc. Portion Weight", f"{total_calculated_weight:.{DISPLAY_PRECISION}f} g", delta=f"{weight_delta:.{DISPLAY_PRECISION}f} g vs Input", delta_color=delta_color, help="Sum of portion weights (excludes trims, kerf loss).")
                 st.caption(f"Density: {calc_results.get('density',0)*1e6:,.2f} kg/m³ | Vol: {calc_results.get('total_volume',0)/1e3:,.1f} cm³", help="Density from Input Weight & Est. Total Volume.")
        ...
        with tab2: # 2D Plots
             ...
             volume_profile = calc_results.get("volume_profile",{})
             sorted_y_starts = calc_results.get("sorted_y_starts",np.array([]))
             density = calc_results.get("density",0)
             if volume_profile and sorted_y_starts.size > 0:
                 col_plot1, col_plot2 = st.columns(2)
                 with col_plot1:
                     with st.spinner("Generating area plot..."):
                         fig_area = plot_area_profile(volume_profile, sorted_y_starts, st.session_state.slice_thickness, calc_results.get("calc_start_y"), calc_results.get("calc_end_y"))
                         st.plotly_chart(fig_area, use_container_width=True)
                 with col_plot2:
                     with st.spinner("Generating weight plot..."):
                         fig_weight = plot_cumulative_weight(volume_profile, sorted_y_starts, density, portions, st.session_state.target_weight, st.session_state.weight_tolerance, calc_results.get("calc_start_y"), calc_results.get("calc_end_y"))
                         st.plotly_chart(fig_weight, use_container_width=True)
             else: st.warning("Not enough data generated to display 2D plots.")
