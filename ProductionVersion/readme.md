...
**- Streamlit Results Viewer (view_results.py):** A separate, web-based dashboard for on-demand analysis.
...
It runs in "watcher" mode, continuously scanning an input directory for new point cloud files.
    ...
    - It communicates with a PLC (if enabled) to read parameters and write back results.
     - view_results.py (The Streamlit Viewer):
...
├── view_results.py              # The Streamlit viewer app
|
