...
# Version: 1.3.9 (Features: File Uploads (CSV/XYZ/PCD/PYL/XLSX/XLS), Test Data Gen,
#                 Volume Modes (Hull, Flat Bottom, Top-Down), Reverse Calc,
#                 Waste First, Interpolation Toggle, Kerf, Tolerance, Trims,
#                 Voxel Downsampling, Optional Auto-Downsampling (Random), 
...
MAX_POINTS_AUTO_DOWNSAMPLE = 350000


# --- Point Cloud Generation ---
def generate_test_point_cloud( # Same as v1.3.8
...
# --- Point Cloud Loading ---
def load_point_cloud(uploaded_file): # Same as v1.3.8
    ...
    try:
        ...
        elif df is not None : st.toast(f"Loaded {len(df)} pts from {uploaded_file.name} using Pandas.", icon="✅") # XYZ via Pandas
        return df
...
def estimate_ror_radius_util_o3d(o3d_pcd, k_neighbors, mult): # Same as before
...
def estimate_slice_volume_convex_hull(df_pts, y_s, y_e, flat_bottom, top_down): # Same as before
...
def recalculate_portion_volume(vol_prof, sorted_y_s, slice_inc, p_min_y, p_max_y): # Same as before
    act_vol = 0.0
...
def calculate_cut_portions_reversed( # Same as before
    points_df, total_w, target_w, slice_inc, no_interp, flat_bottom, top_down,
    blade_thick, w_tol, start_trim, end_trim ):
    ...
    if total_w <= FLOAT_EPSILON: res["status"]="Err: Total weight must be >0."; return res
    ...
    try:
        ...
        if tot_vol <= FLOAT_EPSILON*100: raise ValueError(f"TOTAL est. vol ({tot_vol:.2e}mm³) near zero.")
        res["total_volume"] = tot_vol; dens = total_w / tot_vol
        if not (np.isfinite(dens) and dens >0): raise ValueError("Density invalid.")
        res["density"] = dens; sorted_y_s_arr = np.array(sorted(vp_prof.keys()))
...
def plot_3d_loaf(points_df, portions=None, title="Cheese Loaf Point Cloud", y_offset=0.0): # Same as v1.3.8
...
def plot_area_profile(volume_profile, sorted_y_starts, slice_increment_mm, calc_start_y=None, calc_end_y=None): # Same as v1.3.8
...
def plot_cumulative_weight(volume_profile, sorted_y_starts, density_g_mm3, portions, target_weight, tolerance, calc_start_y=None, calc_end_y=None): # Same as v1.3.8
...
st.set_page_config(layout="wide", page_title="Cheese Portion Calc v1.3.9", page_icon="🧀")
default_persistent_states = {
    ...
    'enable_y_normalization': True, # NEW STATE for Y-Normalization toggle
...
with st.sidebar: # Sidebar UI
    ...
    st.slider("Calc. Slice Thickness (mm)", min_value=0.1, max_value=10.0, key="slice_thickness", step=0.1, format="%.1f")
    st.number_input("Start Trim (mm)", min_value=0.0, key="start_trim", step=1.0, format="%.1f")
    st.number_input("End Trim (mm)", min_value=0.0, key="end_trim", step=1.0, format="%.1f")
    st.number_input("Blade Thickness / Kerf (mm)", min_value=0.0, key="blade_thickness", step=0.1, format="%.1f")
    ...
    st.number_input("Voxel Downsample Size (mm)", min_value=0.0, key="voxel_size", step=0.25, format="%.2f")
    if st.session_state.voxel_size > 0 and not _open3d_installed: st.warning("Voxel downsampling needs `open3d`.")
    st.checkbox("Enable Auto-Downsample (>200k pts)", key="enable_auto_downsample", help="Uses Random Sampling. Uncheck if too slow or not desired.")
    # NEW CHECKBOX for Y-NORMALIZATION
    st.checkbox("Normalize Y-coords before Calculation", key="enable_y_normalization", help="If checked, Y-coordinates will be shifted so the loaf starts near Y=0 for calculation purposes.")
    st.number_input("Scanner Resolution (mm)", min_value=0.01, key="resolution", step=0.1, format="%.2f")
    
    st.subheader("5. Point Cloud Filtering (Optional)") # ROR Section...
        ...
        if not _open3d_installed: st.warning("ROR requires `open3d`.")
            ...
            if st.button("Estimate ROR Radius", use_container_width=True, key="ror_est_btn_widget"):
                if st.session_state.point_cloud_data is not None and not st.session_state.point_cloud_data.empty:
                    ...
                    if not pcd_est_ror.has_points(): st.session_state.ror_estimated_radius_msg = "Cloud empty for ROR est."
                    else:
                ...
                else: st.session_state.ror_estimated_radius_msg = "Load/gen cloud first for ROR est."
            if st.session_state.ror_estimated_radius_msg: st.caption(st.session_state.ror_estimated_radius_msg)
            st.number_input("ROR: Search Radius (mm)", min_value=0.0001, step=0.001, format="%.4f", key="ror_radius_val")
            if st.button("Apply ROR Filter", type="secondary", use_container_width=True, key="ror_apply_btn_widget"):
                if st.session_state.point_cloud_data is not None and not st.session_state.point_cloud_data.empty:
                    if st.session_state.ror_radius_val > 0:
                        try:
                            ...
                            if not pcd_apply_ror.has_points(): st.warning("Cannot apply ROR: Cloud empty.")
                            else:
                                ...
                                st.session_state.ror_filter_applied_msg = (f"ROR Applied: Removed {n_rem} pts. Now: {n_a:,}.")
                ...
                else: st.warning("Load/gen cloud first for ROR.")
...
if st.session_state.data_source == "Generate Test Data":
    ...
    if current_file_id_main_ui != st.session_state.get("last_file_id"): refresh_data_flag_main_ui = True

if refresh_data_flag_main_ui: # BLOCK 1: Initial Data Load & Preprocessing
    ...
    if st.session_state.data_source == "Generate Test Data":
        ...
        if temp_df_load_main_ui is not None:
            ...
            if st.session_state.enable_auto_downsample and len(temp_df_load_main_ui) > MAX_POINTS_AUTO_DOWNSAMPLE: 
                current_step +=1; spinner_msg_auto_ds = f"Step {current_step}/{total_steps}: Auto-downsampling (Random)..."
                with st.spinner(spinner_msg_auto_ds):
                    if _open3d_installed:
                        try:
                            ...
                            ratio_auto_ui = MAX_POINTS_AUTO_DOWNSAMPLE / len(pcd_auto_ds_ui.points); ratio_auto_ui = max(min(ratio_auto_ui, 1.0), 0.00001)
                            pcd_down_auto_ui = pcd_auto_ds_ui.random_down_sample(ratio_auto_ui); method_auto_ui = f"Random (target ratio {ratio_auto_ui:.3f})"
                            if pcd_down_auto_ui and len(pcd_down_auto_ui.points)>0: temp_df_load_main_ui=pd.DataFrame(np.asarray(pcd_down_auto_ui.points), columns=['x','y','z']); st.toast(f"Auto-downsampled to {len(temp_df_load_main_ui):,} pts ({method_auto_ui}).",icon="📉")
...
if points_df_disp_ui is not None and not points_df_disp_ui.empty:
    ...
    st.session_state.y_min_of_displayed_cloud = points_df_disp_ui['y'].min() 
    ...
    with st.spinner("Generating 3D plot..."):
        plot_title = "Cloud with Cuts" if st.session_state.get('calc_results') and st.session_state.calc_results.get("portions") else "Current Point Cloud"
        ...
        fig_3d_main_plot = plot_3d_loaf(points_df_disp_ui, portions=plot_portions, title=plot_title, y_offset=y_offset_for_plot_display) 
...
if calc_button_main_ui: 
    ...
    if points_df_for_calc_main is not None and not points_df_for_calc_main.empty:
        if np.all(np.isfinite(points_df_for_calc_main[['x','y','z']].values)):
            ...
            y_min_for_normalization = 0.0 # Default if normalization is off
            df_to_calculate_with = points_df_for_calc_main # Default to using as-is
            ...
            if st.session_state.enable_y_normalization:
                ...
                st.toast(f"Calc using Y-coords normalized by {-y_min_for_normalization:.2f}mm. Min Y for calc now ~0.", icon="📏")
            else:
                st.toast("Calculation using original Y-coordinates (Y-Normalization disabled).", icon="📐")
            ...
            with st.spinner("Calculating cuts... This may take a moment..."):
                ...
                st.session_state.calc_results = calculate_cut_portions_reversed(
                    df_to_calculate_with, # Pass the (potentially) Y-normalized DataFrame
                    st.session_state.total_weight, st.session_state.target_weight,
                ...
                if st.session_state.calc_results: 
                    st.session_state.calc_results["y_offset_for_plot"] = y_min_for_normalization # Store offset (0.0 if no norm)
            st.rerun() 
        else:
            st.error("Cannot calc - current cloud has non-finite values. Check filters or data.")
            st.session_state.calc_results = {"status": "Error: Non-finite values in point cloud for calculation."}
    else: st.warning("No point cloud data for calculation. Load or generate data.")

# BLOCK 4: Display Results (remains same as v1.3.8)
...
if calc_res_disp_ui:
    ...
    if portions_disp_ui: 
        ...
        with tab1_ui: 
            ...
            with col_table_ui:
                 if not res_df_tab_ui.empty:
                     ...
                     st.download_button("Download Results CSV",csv_ui,"cheese_portions_results.csv","text/csv",key='download-csv-main-widget')
            ...
            with col_metrics_ui_tab: 
                 ...
                 wt_delta_ui=total_calc_wt_ui - st.session_state.total_weight 
                 delta_col_ui="off" if abs(wt_delta_ui)<0.1 else ("inverse" if wt_delta_ui<0 else "normal") 
                 st.metric("Total Calc. Portion Wt",f"{total_calc_wt_ui:.{DISPLAY_PRECISION}f} g",f"{wt_delta_ui:.{DISPLAY_PRECISION}f} g vs Input",delta_color=delta_col_ui)
                 density_ui,volume_ui=calc_res_disp_ui.get('density',0),calc_res_disp_ui.get('total_volume',0) 
                 st.caption(f"Density: {density_ui*1e6:,.2f} kg/m³ | Total Est. Loaf Vol: {volume_ui/1e3:,.1f} cm³")
        ...
        with tab2_ui: 
         ...
         st.warning("Calc ran but gave 0 portions. Check params.")
elif not calc_button_main_ui and st.session_state.get('point_cloud_data') is not None : 
     st.info("Cloud loaded. Set params & click 'Calculate Portions'.")
