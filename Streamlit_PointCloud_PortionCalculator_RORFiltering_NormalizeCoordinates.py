...
def calculate_cut_portions_reversed(
    ...
    try:
        ...
        if direct_density_g_mm3 is not None and direct_density_g_mm3 > FLOAT_EPSILON:
            ...
            if tot_vol <= FLOAT_EPSILON*100: raise ValueError(f"Est. total volume ({tot_vol:.2e}mm³) is near zero. Check area calc method or cloud.")
...
o3d_animation_globals = {
    "current_step": 0, "total_steps": 720, "animation_type": None,
    "pcd_center": np.array([0,0,0]), "initial_view_set": False
}
def reset_o3d_animation_globals():
    o3d_animation_globals["current_step"] = 0; o3d_animation_globals["animation_type"] = None
    o3d_animation_globals["initial_view_set"] = False
def o3d_animation_callback(vis):
    state = o3d_animation_globals; view_control = vis.get_view_control()
    if not state["initial_view_set"]:
        view_control.set_zoom(0.7); view_control.set_lookat(state["pcd_center"])
        state["initial_view_set"] = True
    if state["current_step"] >= state["total_steps"]: return False
    if state["animation_type"] == 'o3d_fly_around': view_control.rotate(5.0, 0.0)
    state["current_step"] += 1; return True
def start_o3d_visualization(points_df, animation_type_str):
    if not _open3d_installed: st.error("Open3D library is not installed."); return
    if points_df is None or points_df.empty: st.warning("No point cloud data to visualize."); return
    pcd = o3d.geometry.PointCloud(); pcd.points = o3d.utility.Vector3dVector(points_df[['x', 'y', 'z']].values)
    if not pcd.has_points(): st.warning("Point cloud empty for Open3D."); return
    reset_o3d_animation_globals(); o3d_animation_globals["animation_type"] = animation_type_str
    o3d_animation_globals["pcd_center"] = pcd.get_center(); o3d_animation_globals["total_steps"] = 720
    window_title = "Open3D Fly Around"; st.info(f"Launching {window_title}. Close to resume.")
    try:
        vis = o3d.visualization.Visualizer(); vis.create_window(window_name=window_title, width=1280, height=720)
        vis.add_geometry(pcd); opt = vis.get_render_option(); opt.point_size = 2.5
        opt.background_color = np.asarray([0.1, 0.1, 0.1]); vis.register_animation_callback(o3d_animation_callback)
        vis.run(); vis.destroy_window()
    except Exception as e: st.error(f"Failed to launch Open3D window: {e}")
    finally: st.info("Open3D window closed."); reset_o3d_animation_globals()
def create_o3d_plane_mesh(y_pos, x_min, x_max, z_min, z_max, color, plane_thickness_along_y=1.0):
    ...
    mesh_box = o3d.geometry.TriangleMesh.create_box(width=(x_max - x_min), height=plane_thickness_along_y, depth=(z_max - z_min))
    mesh_box.translate([x_min, y_pos - (plane_thickness_along_y / 2), z_min]); mesh_box.paint_uniform_color(color)
    return mesh_box
def launch_o3d_viewer_with_cuts(points_df, portions_data, calc_start_y_from_res, calc_end_y_from_res, y_offset_to_add_back):
    if not _open3d_installed: st.error("Open3D library is not installed."); return
    if points_df is None or points_df.empty: st.warning("No point cloud data for Open3D cuts view."); return
    if not portions_data: st.warning("No portion data for Open3D cuts view."); return
    pcd_o3d = o3d.geometry.PointCloud(); pcd_o3d.points = o3d.utility.Vector3dVector(points_df[['x', 'y', 'z']].values)
    if not pcd_o3d.has_points(): st.warning("Point cloud empty for Open3D visualization."); return
    geometries = [pcd_o3d]; x_min_viz, x_max_viz = points_df['x'].min(), points_df['x'].max()
    ...
    plane_thick = max(1.0, (points_df['y'].max() - points_df['y'].min()) * 0.002)
    if calc_start_y_from_res is not None:
        start_plane = create_o3d_plane_mesh(calc_start_y_from_res + y_offset_to_add_back, plane_x_min, plane_x_max, plane_z_min, plane_z_max, color_trim_cut, plane_thick)
        if start_plane: geometries.append(start_plane)
    if calc_end_y_from_res is not None:
        end_plane = create_o3d_plane_mesh(calc_end_y_from_res + y_offset_to_add_back, plane_x_min, plane_x_max, plane_z_min, plane_z_max, color_trim_cut, plane_thick)
        if end_plane: geometries.append(end_plane)
    for i, p_item in enumerate(portions_data):
        cut_y_val_calc = p_item.get('cut_y')
        if cut_y_val_calc is not None:
            is_s_trim = (calc_start_y_from_res is not None and abs(cut_y_val_calc - calc_start_y_from_res) < FLOAT_EPSILON)
            is_e_trim = (calc_end_y_from_res is not None and abs(cut_y_val_calc - calc_end_y_from_res) < FLOAT_EPSILON)
            if not is_s_trim and not is_e_trim and i < len(portions_data) -1 :
                eff_cut_y_orig = cut_y_val_calc + y_offset_to_add_back
                portion_plane = create_o3d_plane_mesh(eff_cut_y_orig, plane_x_min, plane_x_max, plane_z_min, plane_z_max, color_portion_cut, plane_thick)
                if portion_plane: geometries.append(portion_plane)
    st.info("Launching Open3D viewer with cuts. Close window to resume.")
    try:
        o3d.visualization.draw_geometries(geometries, window_name="Open3D - Point Cloud with Cuts", width=1280, height=720, point_show_normal=False)
    except Exception as e: st.error(f"Failed to launch Open3D window: {e}")
...
with st.sidebar:
            ...
            with st.spinner("Preparing Binary PLY download..."):
                try:
                    ...
                    with open(tmp_ply_path, "rb") as fp:
                        st.download_button(label="Click to Download Binary PLY", data=fp, file_name="converted_cloud.ply", mime="application/octet-stream")
    ...
    if st.session_state.density_source == "Calculate from Total Weight & Volume":
        ...
        st.number_input("Direct Density (g/cm³)", min_value=0.01, key="direct_density_g_cm3", step=0.01, format="%.3f", help="Water is 1 g/cm³.")
    ...
    st.caption("Profile scan type (e.g. 4 profilers) presumed unless specific options below are checked.")
    ...
    st.number_input("Auto-Downsample Threshold (points)", min_value=50000, max_value=5000000, step=50000, key="auto_downsample_threshold", disabled=not st.session_state.enable_auto_downsample)
    st.checkbox("Normalize Y-coords Before Calculation", key="enable_y_normalization")
...
if points_df_disp_ui is not None and not points_df_disp_ui.empty:
    ...
    try:
        if np.all(np.isfinite(points_df_disp_ui[['x','y','z']].values)):
            ...
            if st.session_state.enable_auto_downsample and 'original_point_count' in st.session_state and st.session_state.original_point_count > st.session_state.auto_downsample_threshold:
                col1_met_ui.caption(f"Before Downsample: {st.session_state.get('original_point_count', 0):,} pts")
    ...
    if not _open3d_installed:
        st.warning("Open3D library is not installed. External inspection features are disabled.", icon="⚠️")
        ...
        if np.all(np.isfinite(points_df_for_calc_main[['x','y','z']].values)):
            ...
            with st.spinner("Calculating cuts... This may take a moment..."):
                ...
                if st.session_state.density_source == "Input Directly":
                    ...
                    else: st.error("Direct density selected, but value is invalid."); st.session_state.calc_results = {"status": "Error: Invalid direct density."}; st.rerun()
                else:
...
if calc_res_disp_ui:
    ...
    if "Err:" in stat_msg_ui: st.error(stat_msg_ui)
    elif "Warning:" in stat_msg_ui: st.warning(stat_msg_ui)
    else: st.info(stat_msg_ui)
    if density_source_msg_ui and "Err:" not in stat_msg_ui : st.caption(density_source_msg_ui)
    ...
    if portions_disp_ui:
        ...
        with tab1_ui:
                 ...
                 if not res_df_tab_ui.empty:
                     ...
                     with btn_cols[1]:
                         ...
                         if st.button("👁️ View Cuts in 3D (Open3D)", key="o3d_view_cuts_btn", use_container_width=True, disabled=not _open3d_installed or current_display_cloud is None):
                             ...
                             else: st.warning("No point cloud data available for 3D view with cuts.")
                         if not _open3d_installed: st.caption("Open3D view disabled (library not found).")
            ...
            with col_metrics_ui_tab:
                 ...
                 else: st.metric("Total Calc. Portion Wt",f"{total_calc_wt_ui:.{DISPLAY_PRECISION}f} g")
